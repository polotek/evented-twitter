...
var DEFAULT_OPTIONS = {authRequired:true
                      , 'errHandler': function(err) { throw err; }
                      };
...
function addAPIMethod(ctx, urlPath, name, default_options, default_params) {
    sys.puts('  api method: ' + name);

    var urlPath = urlPath;

    default_options = _.extend({}, DEFAULT_OPTIONS, default_options);
    sys.puts('  o:' + sys.inspect(default_options));
 
    var default_params = default_params || {};
    default_params = _.extend({}, DEFAULT_PARAMS, default_params);
    ...
    ctx.prototype[name] = function(format, params, cb) {
        ...
        if(params) {
            ...
            if(default_params.screen_name && this._username) delete params.screen_name;
        } else if(default_params.screen_name && !default_options.authRequired) {
            params = {screen_name:this._username};
        }
